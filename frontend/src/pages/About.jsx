...
function About() {
  ...
  const fetchArtistPhoto = async () => {
    try {
      const response = await axios.get('/api/v1/artist_profile')
          ...
          <div className="about-container">
            <div className="about-image-section">
              <div className="artist-image-placeholder">
                {artistPhoto ? (
                  <img src={artistPhoto} alt={t('artist.name')} className="artist-photo" />
                ) : (
                  <div className="artist-initials">MQ</div>
                )}
                <div className={`artist-name-overlay ${!nameVisible ? 'show' : ''}`}>
                  <h2 className="artist-name-overlay-text">{t('artist.name')}</h2>
                  <p className="artistic-name-overlay">{t('about.artisticName')}: <strong>{t('artist.artisticName')}</strong></p>
                </div>
              </div>
            </div>
            
            <div className="about-text-section">
              <h2 className="artist-name">{t('artist.name')}</h2>
              <p className="artistic-name">{t('about.artisticName')}: <strong>{t('artist.artisticName')}</strong></p>
              
              <div className="artist-details">
                <div className="detail-item">
                  <span className="detail-label">{t('about.birthdate')}:</span>
                  <span className="detail-value">{t('artist.birthdate')}</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">{t('about.country')}:</span>
                  <span className="detail-value">{t('artist.country')}</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">{t('about.technique')}:</span>
                  <span className="detail-value">{t('artist.technique')}</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">{t('about.location')}:</span>
                  <span className="detail-value">{t('artist.location')}</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">{t('about.email')}:</span>
                  <span className="detail-value">
                    <a href={`mailto:${t('artist.email')}`}>{t('artist.email')}</a>
                  </span>
                </div>
                <div className="detail-item">
